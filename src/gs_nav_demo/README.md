...
- 订阅导航 `map_server` 的 `/map` 栅格地图，在左下角绘制地图和全局路径；
...
- 导航页左下角可在栅格地图与俯视点云地图之间即时切换；
- 有相机内参和 TF 时执行真实针孔投影，尚未标定时自动回退到可调地面投影。

## 无硬件预览

```bash
cd ~/ws/car_simple_sim
...
切换到实时画面。右侧地图每次点击会依次追加一个途径点，机器人从当前 TF 位置出发，
按编号顺序前往各点。点击“开始导航”后进入纯导航页面，仅保留相机、AR 路线、左下角
小地图和底部“退出导航”按钮。退出会取消整个 `/navigate_through_poses` 任务。如果通过
SSH 启动，需要保证图形转发可用并且环境中存在 `DISPLAY`。
...
放大缩小；双击点云区域或点击“重置三维视角”恢复默认等轴视角。导航页中的点云仍然
保持俯视小地图形式，便于选择途径点和观察全局路径。
...
栅格；保存当前点云时统一输出 binary PCD，并保留已加载的 RGB。转换仍使用原始 XYZ
和界面设置的原始 Z 高度范围。选择点云添加/擦除画笔时，界面会
自动回到原始 XYZ 显示，避免在显示变换下误编辑。
...
显示矩阵；转栅格默认选择“当前显示坐标（副本）”，原始 PLY 不会被旋转或覆盖。
