...
namespace scan_planner
{
  ...
  void SCANReplanFSM::init(rclcpp::Node *node)
  {
    ...
    execution_frozen_sub_ = node_->create_subscription<std_msgs::msg::Bool>(
        "planning/execution_frozen", 10,
        std::bind(&SCANReplanFSM::executionFrozenCallback, this, std::placeholders::_1));
  ...
  void SCANReplanFSM::executionFrozenCallback(const std_msgs::msg::Bool::ConstSharedPtr &msg)
  {
    execution_frozen_ = msg->data;
  }
  ...
  void SCANReplanFSM::updateLocalTrajTimeFreeze()
  {
    ...
    if (execution_frozen_ && info->start_time_.seconds() > 1e-5)
