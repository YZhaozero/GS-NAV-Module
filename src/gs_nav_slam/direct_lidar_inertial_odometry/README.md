...
This workspace is preconfigured for the Livox MID-360 driver:

- point cloud: `/livox/lidar/pointcloud`
- IMU: `/livox/imu`
- LiDAR and IMU frame: `livox_frame`
- odometry and base frames: `odom` and `base_link`
- system clock by default (`use_sim_time:=false`)

Start the driver and DLIO in separate terminals:

```sh
ros2 launch livox_ros_driver2 msg_MID360_launch.py xfer_format:=4
ros2 launch direct_lidar_inertial_odometry dlio.launch.py rviz:=true
```

For rosbag playback, start DLIO with `use_sim_time:=true`.
