...
T getRemoteParam(const rclcpp::Node::SharedPtr &nh, const std::string& remote_node_name, const std::string& param_name, const T& defaultValue)
{
    ...
    try {
        ...
        // 冷启动竞态修复: 100ms 内 DDS 常来不及发现 parameter_blackboard 的服务,
        // 导致 loadFromRosNs 取到空 cam_model 而 throw("Camera model not correctly
        // specified.")。提到 3000ms 让首次启动也能等到服务。
        if (parameters_client->wait_for_service(std::chrono::milliseconds(3000))) {
